 ...
 *
 * @param channel     -1 means any channel
 * @param minVelocity Minimum velocity to trigger (1-127)
 * @param maxVelocity Maximum velocity to trigger (1-127)
 */
public record NoteMapping(MidiNote midiNote, KeyCombination keyCombination, int channel, int minVelocity,
                          int maxVelocity) {
    ...
    public boolean matches(int noteNumber, int channel) {
        return midiNote.noteNumber() == noteNumber &&
    ...
    public boolean conflictsWith(NoteMapping other) {
        ...
        // Same note number
        if (midiNote.noteNumber() != other.midiNote.noteNumber()) {
            return false;
        }
    ...
    public NoteMapping copy() {
        return new NoteMapping(
                new MidiNote(midiNote.noteNumber()),
                keyCombination.copy(),
                channel,
                minVelocity,
                maxVelocity
