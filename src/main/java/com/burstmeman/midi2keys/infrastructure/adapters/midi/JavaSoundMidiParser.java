...
public class JavaSoundMidiParser implements MidiParser {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JavaSoundMidiParser.class);
    ...
    public MidiFileInfo parse(Path path) throws IOException, MidiParseException {
        logger.debug("Parsing MIDI file: {}", path);
    ...
    public boolean isValidMidiFile(Path path) {
        try {
            ...
            logger.debug("File is not a valid MIDI: {} - {}", path, e.getMessage());
            return false;
        }
    }
    ...
    private MidiFileInfo parseSequence(Sequence sequence) throws MidiParseException {
        ...
        logger.info("Parsed MIDI: format={}, tracks={}, duration={}ms, tempo={} BPM, notes={}",
