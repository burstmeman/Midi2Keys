...
public class RobotKeyboardSimulator implements KeyboardSimulator {
    ...
    public RobotKeyboardSimulator() {
        ...
        try {
            ...
            isOperational = true;
            log.info("RobotKeyboardSimulator initialized successfully");
        } catch (AWTException e) {
            log.error("Failed to initialize Robot - keyboard simulation will not work", e);
        } catch (SecurityException e) {
            log.error("Security exception - keyboard simulation not permitted", e);
        }
    ...
    public void keyPress(int keyCode) {
        if (!operational || robot == null) {
            log.warn("Robot not operational, ignoring keyPress: {}", keyCode);
            return;
        }
        ...
        try {
            ...
            log.warn("Invalid key code: {}", keyCode);
        }
    }
    ...
    public void keyRelease(int keyCode) {
        if (!operational || robot == null) {
            log.warn("Robot not operational, ignoring keyRelease: {}", keyCode);
            return;
        }
    ...
    public void releaseAllKeys() {
        log.info("Releasing all {} pressed keys", pressedKeys.size());
        ...
        for (int keyCode : keysToRelease) {
            try {
                ...
                log.warn("Failed to release key {}: {}", keyCode, e.getMessage());
            }
        }

        pressedKeys.clear();
        log.debug("All keys released");
    }
