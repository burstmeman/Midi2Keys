...
public class ProfileJsonSerializer {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ProfileJsonSerializer.class);
    ...
    private PlaybackOptions deserializePlaybackOptions(JsonNode node) {
        ...
        if (node.has("quantization")) {
            try {
                ...
                logger.warn("Unknown quantization value, using NONE");
            }
        }
    ...
    private ArrayNode serializeNoteMappings(List<NoteMapping> mappings) {
        ...
        for (NoteMapping mapping : mappings) {
            ...
            node.put("noteNumber", mapping.getMidiNote().getNoteNumber());
            node.put("noteName", mapping.getMidiNote().getNoteName());
            node.put("keyCombination", mapping.getKeyCombination().getDisplayString());
            node.put("channel", mapping.getChannel());
            node.put("minVelocity", mapping.getMinVelocity());
            node.put("maxVelocity", mapping.getMaxVelocity());
    ...
    private List<NoteMapping> deserializeNoteMappings(JsonNode node) {
        ...
        for (JsonNode mappingNode : node) {
            try {
                ...
                logger.warn("Failed to deserialize mapping: {}", e.getMessage());
            }
        }
    ...
    private LocalDateTime parseDateTime(JsonNode root, String field) {
        if (root.has(field) && !root.get(field).isNull()) {
            try {
                ...
                logger.warn("Failed to parse datetime field {}: {}", field, e.getMessage());
            }
        }
