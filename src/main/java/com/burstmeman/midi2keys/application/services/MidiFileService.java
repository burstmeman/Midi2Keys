...
public class MidiFileService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MidiFileService.class);
    ...
    public int scanRootDirectory(RootDirectory rootDirectory) {
        logger.info("Scanning root directory: {}", rootDirectory.getPath());
        
        Path rootPath = rootDirectory.getPathAsPath();
        int count = scanDirectory(rootDirectory.getId(), rootPath, rootPath);
        
        logger.info("Scan complete. Found {} MIDI files in {}", count, rootDirectory.getPath());
        return count;
    }
    ...
    public void updateNoteShift(Long midiFileId, int noteShift) {
        ...
        logger.debug("Updated note shift for file {}: {}", midiFileId, noteShift);
    }
    ...
    private int scanDirectory(Long rootDirectoryId, Path rootPath, Path currentPath) {
        ...
        try {
            ...
            logger.warn("Failed to scan directory: {} - {}", currentPath, e.getMessage());
        }
    ...
    private MidiFile getOrCreateMidiFile(Long rootDirectoryId, String relativePath, Path absolutePath) {
        ...
        if (existing.isPresent()) {
            ...
            try {
                ...
                logger.warn("Failed to update file metadata: {}", relativePath);
            }
        ...
        try {
            ...
            logger.warn("Failed to get file metadata: {}", relativePath);
        }
