...
public class PanicStopService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PanicStopService.class);
    ...
    public void triggerPanicStop() {
        ...
        logger.warn("PANIC STOP triggered - releasing all keys and stopping playback");
        ...
        if (playbackService != null) {
            try {
                ...
                logger.error("Error stopping playback during panic stop", e);
            }
        }
        ...
        if (onPanicStop != null) {
            Platform.runLater(() -> {
                try {
                    ...
                    logger.error("Error in panic stop callback", e);
                }
            });
        }
        ...
        logger.info("Panic stop completed - all keys released");
    }
     ...
     */
    public void setOnPanicStop(Consumer<Void> callback) {
        this.onPanicStop = callback;
    }
