...
import com.burstmeman.midi2keys.infrastructure.adapters.midi.MidiParser.MidiTrackInfo;
...
public class MidiAnalysisService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MidiAnalysisService.class);
    ...
    public MidiAnalysis getOrAnalyze(MidiFile midiFile, RootDirectory rootDirectory) {
        ...
        if (cached.isPresent()) {
            logger.debug("Using cached analysis for: {}", midiFile.getFileName());
            return cached.get();
        }
    ...
    public MidiAnalysis analyze(MidiFile midiFile, RootDirectory rootDirectory) {
        ...
        logger.info("Analyzing MIDI file: {}", filePath);
        ...
        try {
            ...
            logger.info("Analysis complete: {} notes, {} tracks, {}ms", 
