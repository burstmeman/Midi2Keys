...
public class PlayMidiFileUseCase {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PlayMidiFileUseCase.class);
    ...
    private void cancelCountdown() {
        if (countdownTimer != null) {
            countdownTimer.cancel();
            countdownTimer = null;
            logger.debug("Countdown cancelled");
        }
    }
    
    private void startPlaybackInternal(MidiFile midiFile, RootDirectory rootDir, Profile profile) {
        logger.info("Starting playback: {} with profile {}", midiFile.getFileName(), profile.getName());
